...
public class EmailAddressUnitTests
{
    ...
    public void
        SimplerWayToTestSameScenarioAsAbove_GivenProperEmail_WhenAnonChoosesToRegister_TheEmailAddressIsValidated(
    ...
    [Fact]
    public void
        GivenEmailDomainIncorrect_WhenAnonChoosesToRegisterWithOtherwsieValidValues_ThenTheRequestIsRejectedWithProperErrorMessage()
    {
        string testEmail = "abcd@gmail.com";
        Result<EmailAddress> result = EmailAddress.Create(testEmail);

        Assert.True(result.IsFailure);
        Assert.Contains(Error.EmailNotEndingWithViaDk.Message, result.Errors[0].Message,
    ...
    [Fact]
    public void
        GivenEmailFormatIncorrect_WhenAnonChoosesToRegisterWithOtherwsieValidValues_ThenTheRequestIsRejectedWithProperErrorMessages()
    {
        ...
        Assert.True(result.IsFailure);
        Assert.NotEmpty(result.Errors);
        Assert.True(result.Errors.Count == 4, "Should contain 4 errors.");

        // Optional: check specific error messages
        var expectedErrors = new[]
        {
            Error.EmailNotEndingWithViaDk,
            Error.EmailDoesNotFollowFormatText1AtText2DotText3,
            Error.EmailPartBeforeAtMustBeBetween3And6CharactersLong,
            Error.EmailPartBeforeAtMustBeEither3Or4LettersOr6Digits
        };
    ...
    [InlineData("    ")]
    public void
        GivenEmptyOrWhitespaceEmail_WhenAnonChoosesToRegisterWithOtherwsieValidValues_ThenTheRequestIsRejectedWithProperErrorMessage(
            string? input)
    {
        var result = EmailAddress.Create(input!); // the method expects non-null, so we use ! to force it

        Assert.True(result.IsFailure);
        Assert.Single(result.Errors); // Expects that the list of errors is of lenght 1, no more, no less
        Assert.Contains(Error.EmailRequired, result.Errors);
    }
}