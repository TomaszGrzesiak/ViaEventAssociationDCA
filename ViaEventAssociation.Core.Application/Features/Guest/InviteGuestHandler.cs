...
public class InviteGuestHandler(IEventRepository eventRepository, IGuestRepository guestRepository, IUnitOfWork unitOfWork)
    : ICommandHandler<InviteGuestCommand>
{
    public async Task<Result> HandleAsync(InviteGuestCommand command)
    {
        ...
        // Business change
        var result = ev.InviteGuest(command.GuestId);
