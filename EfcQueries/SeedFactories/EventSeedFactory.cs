...
namespace EfcQueries.SeedFactories;

public static class EventSeedFactory
{
    public static List<VeaEvent> CreateEvents()
    {
        List<TmpEvent> eventTmps =
            JsonSerializer.Deserialize<List<TmpEvent>>(EventsAsJson)!;

        List<VeaEvent> events = eventTmps
            .Select(evt => new VeaEvent
            {
                Id = evt.Id,
                TitleValue = evt.Title,
                DescriptionValue = evt.Description,
                Status = ConvertStatus(evt.Status),
                Visibility = ConvertVisibility(evt.Visibility),
                TimeRangeStartTime = evt.Start,        // matches your scaffold
                TimeRangeEndTime = evt.End,
                MaxGuestsNoValue = evt.MaxGuests
            })
            .ToList();
    ...
    private static int ConvertStatus(string status) =>
      status.ToLower() switch
      {
        ...
        "ready"  =>    (int)EventStatus.Ready,
        "cancelled" => (int)EventStatus.Cancelled,
        "draft"     => (int)EventStatus.Draft,
        _ => throw new Exception($"Unknown event status: {status}")
      };

    private static int ConvertVisibility(string visibility) =>
      visibility.ToLower() switch
      {
        "public"  => (int)EventVisibility.Public,
        "private" => (int)EventVisibility.Private,
        _ => throw new Exception($"Unknown visibility: {visibility}")
      };
    
    // 👇 Paste the content of Events.json here
