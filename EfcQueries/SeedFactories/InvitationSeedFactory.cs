...
{
    public static void Seed(VeaReadModelContext context)
    {
        List<TempInvitation> tmpInvitations =
            JsonSerializer.Deserialize<List<TempInvitation>>(InvitationsAsJson)!;

        var invitations = tmpInvitations.Select(t => new Invitation
        {
            InvitationId = Guid.NewGuid().ToString(),   // PK, JSON doesn’t have it
            GuestId = t.GuestId,
            EventId = t.EventId,
    ...
    private static int ConvertStatus(string status) =>
        ...
        {
            "pending"  => 1, // InvitationStatus.Pending
            "accepted" => 2, // InvitationStatus.Accepted
            "declined" => 3, // InvitationStatus.Declined
            _ => throw new ArgumentOutOfRangeException(
                nameof(status), status, "Unknown invitation status value")
        };
