...
public static class ParticipationSeedFactory
{
  ...
  {
    List<TempParticipation> participations =
      JsonSerializer.Deserialize<List<TempParticipation>>(ParticipationsAsJson)!;

    // create join rows directly
    var entities = participations.Select(p => new EventParticipant
    {
      EventId = p.EventId,
      GuestId = p.GuestId
    });

    context.EventParticipants.AddRange(entities);
  }
